...
public class DomicilioController {
	...
	private  Credentials getCliente() {
       ...
       public String aggiungiDomicilio(@ModelAttribute("domicilio") Domicilio domicilio, BindingResult result, Model model) {
          
    	  Credentials c= getCliente();
    	  Cliente cliente= c.getUser();
    	  this.domicilioValidator.validate(domicilio, result);
    	  if (result.hasErrors()) {
       ...
       public String deleteAdd(Model model, @ModelAttribute("domicilio") Domicilio dom) {
    	   ...
    	   Credentials c=getCliente();
   		   Cliente cliente=c.getUser();
   		   List<Ordine> ordiniPerDomicilio = this.ordineService.getOrdinePerDomicilio(domicilio);
   		   if (ordiniPerDomicilio.size() > 0) {
